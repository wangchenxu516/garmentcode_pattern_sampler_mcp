...
def get_command_args():
    ...
    parser.add_argument('--json_output_path', type=str, default=None, help='Path to write a JSON file with the final dataset path.')
...
def generate(path, properties, sys_paths, verbose=False):
    ...
    for i in range(properties['size']):
        ...
        # Redo sampling until success
        max_retries = int(os.environ.get('MAX_RETRIES', '100'))
        for _ in range(max_retries):  # limit retries via env to avoid long runs under MCP
            ...
            try:
                ...
                if verbose:
                    traceback.print_exc()
                # Sanitize error message to prevent encoding crashes
                sanitized_error = str(e).encode('ascii', 'replace').decode('ascii')
    ...
    # Return dataset path
    return data_folder, default_path, body_sample_path
...
if __name__ == '__main__':
    ...
    # Generator
    data_folder, default_path, body_sample_path = generate(
        system_props['datasets_path'], props, system_props, verbose=False)

    # Gather the pattern images separately (can be disabled via env for faster runs)
    if os.environ.get('DISABLE_GATHER_VISUALS', '0') != '1':
        gather_visuals(default_path)
        gather_visuals(body_sample_path)

    print('Data generation completed!')

    # SUCCESS: Write final result to the specified JSON file
    if args.json_output_path:
        output_data = {'dataset_path': str(data_folder)}
        with open(args.json_output_path, 'w') as f:
            json.dump(output_data, f)
    else:
        # Fallback for old behavior if no path is provided
        print(f'New patterns saved to {data_folder}')
